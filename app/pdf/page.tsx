...
// PDFエンジンをブラウザ専用として読み込む（サーバーエラー回避）
...
// フォント設定（PDFViewer.tsxと同じもの）
const FONT_OPTIONS = [
  { label: '丸ゴシック', value: 'gothic.ttf' },
  { label: '明朝体', value: 'mincho.ttf' },
  { label: '筆文字', value: 'brush.ttf' },
];

function PDFEditorContent() {
  const searchParams = useSearchParams();
  const docId = searchParams.get('id'); // URLからIDを取得（クラウド編集用）
  ...
  const [saveFolderName, setSaveFolderName] = useState('PDF編集'); // デフォルトフォルダ
      ...
      const loadFromCloud = async () => {
        ...
        if (error || !data) {
          ...
          try {
            ...
            setSaveFolderName(data.folder_name || 'PDF編集');
          } catch (e) { alert('ファイルデータの読み込みに失敗しました'); }
        }
  ...
  // --- 保存実行処理（ダウンロード or クラウド） ---
  const executeSave = async (target: 'download' | 'cloud') => {
    if (!file) return;
    setIsSaving(true);

    try {
      // ★重要: pdf-libをここで動的に読み込む（サーバーエラー回避）
          ...
          if (annot.type === 'text' && annot.content) {
            ...
            page.drawText(annot.content, { 
             ...
             page.drawRectangle({ 
               ...
               borderColor: isWhite ? undefined : drawColor, 
               borderWidth: isWhite ? 0 : Math.max(2, annot.size/3), 
               color: isWhite ? rgb(1,1,1) : undefined 
             });
      ...
      if (savePassword) {
        try { 
          pdfDoc.encrypt({ userPassword: savePassword, ownerPassword: savePassword, permissions: { printing: 'highResolution', modifying: false, copying: false, annotating: false, fillingForms: false, contentAccessibility: false, documentAssembly: false } }); 
        } catch (e) { alert("パスワード設定に失敗しました"); }
      }
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col h-screen text-gray-800">
      ...
      <div className="bg-white border-b p-2 flex gap-2 items-center shadow-sm overflow-x-auto whitespace-nowrap h-14">
        <div className="flex gap-1 border-r pr-2 items-center">
          ...
          {/* 上書き保存（即ダウンロード） */}
          <button onClick={() => executeSave('download')} disabled={!file || isSaving} className="bg-green-50 text-green-700 hover:bg-green-100 px-2 py-1 rounded font-bold text-xs">💾 上書き</button>
          {/* 名前をつけて保存（モーダル） */}
          <button onClick={() => setShowSaveModal(true)} disabled={!file || isSaving} className="bg-yellow-50 text-yellow-700 hover:bg-yellow-100 px-2 py-1 rounded font-bold text-xs">📝 保存設定</button>
        </div>
          ...
          <div className="flex-1 bg-gray-500 p-8 overflow-auto flex justify-center relative">
            {file ? (
              <PDFViewer 
                ...
                currentColor={currentColor} currentSize={currentSize} currentFont={currentFont} showGrid={showGrid} 
      ...
      {showSaveModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-sm">
            <h3 className="text-lg font-bold mb-4">保存設定</h3>
            ...
            <label className="block text-sm font-bold text-gray-700 mb-1">フォルダ名 (クラウド用)</label>
            <input type="text" value={saveFolderName} onChange={e => setSaveFolderName(e.target.value)} list="folders" className="w-full border p-2 rounded mb-4" />
            <datalist id="folders"><option value="PDF編集"/><option value="仕事"/><option value="家事"/></datalist>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowSaveModal(false)} className="flex-1 bg-gray-200 text-gray-800 py-2 rounded font-bold text-sm">キャンセル</button>
              <button onClick={() => executeSave('download')} disabled={isSaving} className="flex-1 bg-green-600 text-white py-2 rounded font-bold text-sm">💾 PCにDL</button>
              <button onClick={() => executeSave('cloud')} disabled={isSaving} className="flex-1 bg-blue-600 text-white py-2 rounded font-bold text-sm">☁ クラウド保存</button>
            </div>
          </div>
        </div>
      )}
