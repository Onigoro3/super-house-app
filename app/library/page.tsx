...
export default function LibraryApp() {
  ...
  const [speechRate, setSpeechRate] = useState(1.0); // 速度
  const [speechVolume, setSpeechVolume] = useState(1.0); // 音量
  const [showSettings, setShowSettings] = useState(false); // 設定パネル開閉

  const audioRef = useRef<HTMLAudioElement | null>(null);
  // 再生中フラグ（Refで管理してイベントハンドラ内で最新値を参照）
  ...
  useEffect(() => {
    ...
    // 標準音声の場合は発話中に変えられないため、次回の発話から反映される
  ...
  const speakStandard = (text: string) => {
    ...
    u.lang = 'ja-JP';
    u.rate = speechRate; // 設定値を反映
    u.volume = speechVolume; // 設定値を反映
    u.onend = handleAudioEnd;
  ...
  const playCurrentContent = async () => {
    ...
    if (useVoicevox) {
      ...
      } catch (e) { speakStandard(textToRead); }
    } else {
      speakStandard(textToRead);
    }
  };
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {displayedBooks.map(book => (
                  <div key={book.id} className="group relative flex flex-col gap-2">
                    <div onClick={() => openBook(book)} className="aspect-[3/4] bg-gradient-to-br from-indigo-900 to-indigo-700 rounded-r-lg shadow-lg cursor-pointer hover:-translate-y-2 transition-transform flex flex-col justify-center items-center border-l-8 border-indigo-950 text-gray-800 relative overflow-hidden p-4 text-center border-2 border-amber-100">
            ...
            <div className="flex flex-col h-full bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
              <div className="bg-[#fdf6e3] p-4 border-b border-amber-100 flex justify-between items-center shrink-0 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={downloadPDF} className="text-xs bg-gray-100 px-2 py-1 rounded hover:bg-gray-200">📑 PDF</button>
                  <div className="w-1 h-6 bg-gray-300 mx-1"></div>
                  
                  {/* ★設定ボタン */}
                  <button onClick={() => setShowSettings(!showSettings)} className="text-xs bg-amber-100 px-2 py-1 rounded hover:bg-amber-200">⚙ 設定</button>
              ...
              {/* ★設定パネル（開閉式） */}
              {showSettings && (
                <div className="bg-[#fff9e6] p-4 border-b border-amber-200 animate-fadeIn">
                  <div className="flex flex-wrap gap-6 items-center justify-center">
                    <label className="flex items-center gap-2 cursor-pointer bg-white px-3 py-1 rounded border border-amber-300">
                      <input type="checkbox" checked={useVoicevox} onChange={() => setUseVoicevox(!useVoicevox)} />
                      <span className="text-sm font-bold text-amber-800">美声モード</span>
                    </label>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-amber-700">速度: {speechRate.toFixed(1)}</span>
                      <input type="range" min="0.5" max="2.0" step="0.1" value={speechRate} onChange={(e) => setSpeechRate(parseFloat(e.target.value))} className="w-24 accent-amber-600" />
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-amber-700">音量: {Math.round(speechVolume * 100)}%</span>
                      <input type="range" min="0" max="1.0" step="0.1" value={speechVolume} onChange={(e) => setSpeechVolume(parseFloat(e.target.value))} className="w-24 accent-amber-600" />
                    </div>
                  </div>
                </div>
              )}
