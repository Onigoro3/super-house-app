...
export default function LibraryApp() {
  ...
  const playCurrentContent = async () => {
    ...
    if (useVoicevox) { try { const url = `https://api.tts.quest/v3/voicevox/synthesis?text=${encodeURIComponent(textToRead)}&speaker=2`; if (audioRef.current) { audioRef.current.src = url; audioRef.current.playbackRate = speechRate; audioRef.current.volume = speechVolume; await audioRef.current.play(); audioRef.current.onended = handleAudioEnd; audioRef.current.onerror = () => { console.warn("VOICEVOX Error, fallback"); speakStandard(textToRead, handleAudioEnd); }; } } catch (e) { speakStandard(textToRead, handleAudioEnd); } } else { speakStandard(textToRead, handleAudioEnd); }
  };
