...
export default function LibraryApp() {
  ...
  const [bookGenre, setBookGenre] = useState('study'); // 変数名をgenreに変更
  ...
  const generateBook = async () => {
    ...
    try {
      let finalPages: BookPage[] = [];
      let finalTitle = "";
      
      // 長さに応じてループ回数を決定
      // short: 1回(約5-8頁), long: 2回(約20頁), super_long: 4回(約40頁)
      const loops = bookLength === 'short' ? 1 : (bookLength === 'long' ? 2 : 4);
      
      for (let i = 1; i <= loops; i++) {
        setGenerateStatus(`執筆中... (パート ${i} / ${loops})`);
        
        // 前回の最後の文章を文脈として渡す
        const previousContent = finalPages.length > 0 ? finalPages[finalPages.length - 1].content : "";
        ...
        if (!res.ok) throw new Error(`パート${i}の生成に失敗`);
        const data = await res.json();
        
        if (i === 1) finalTitle = data.title; // 最初のタイトルを採用

        // ページ番号を調整して追加
        const startPageNum = finalPages.length + 1;
        const newPages = data.pages.map((p: any, idx: number) => ({
          ...p,
          page_number: startPageNum + idx
        }));
        
        finalPages = [...finalPages, ...newPages];
        
        // 少し休憩（API制限回避のため）
        if (i < loops) await new Promise(r => setTimeout(r, 1000));
      }
      ...
      if (!error) {
        alert(`『${finalTitle}』が出版されました！(全${finalPages.length}ページ)`);
          ...
          {view === 'create' && <div className="bg-white p-6 rounded-lg shadow-sm border border-amber-200"><div className="flex justify-between items-center mb-4"><h2 className="font-bold text-lg text-amber-900">✨ 新しい本を執筆</h2><button onClick={() => setView('shelf')} className="text-sm text-gray-400">キャンセル</button></div><div className="flex flex-col gap-4"><input type="text" value={topic} onChange={e => setTopic(e.target.value)} placeholder="テーマ (例: 銀河鉄道の夜のような物語)" className="border p-3 rounded-lg w-full bg-amber-50" />
            <div className="flex gap-2">
              ...
              <select value={bookLength} onChange={e => setBookLength(e.target.value)} className="border p-3 rounded-lg bg-white">
                ...
                <option value="long">中編 (20頁)</option>
