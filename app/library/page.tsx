...
export default function LibraryApp() {
  ...
  const speakStandard = (text: string) => { if (typeof window === 'undefined') return; window.speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(text); u.lang = 'ja-JP'; u.rate = speechRate; u.volume = speechVolume; u.onend = handleAudioEnd; window.speechSynthesis.speak(u); };
  
  const playCurrentContent = async () => {
    if (!currentBook) return;
    const page = currentBook.pages[currentPageIndex];
    // 見出しは読まず、本文のみ
    const textToRead = currentBook.topic === 'インポート' ? cleanText(`${page.headline}。\n${page.content}`) : cleanText(page.content);
    if (useVoicevox) { try { const url = `https://api.tts.quest/v3/voicevox/synthesis?text=${encodeURIComponent(textToRead)}&speaker=2`; if (audioRef.current) { audioRef.current.src = url; audioRef.current.playbackRate = speechRate; audioRef.current.volume = speechVolume; await audioRef.current.play(); audioRef.current.onended = handleAudioEnd; audioRef.current.onerror = () => { console.warn("VOICEVOX Error, fallback"); speakStandard(textToRead); }; } } catch (e) { speakStandard(textToRead); } } else { speakStandard(textToRead); }
  };
  ...
  const changePage = (newIndex: number) => { stopSpeaking(); setCurrentPageIndex(newIndex); if (currentBook) saveBookmark(currentBook.id, newIndex); };
  ...
  return (
    <div className="min-h-screen bg-amber-50 flex flex-col h-screen text-gray-800 font-serif relative overflow-hidden">
      ...
      <header className="bg-amber-900 text-white p-4 shadow-md flex justify-between items-center z-10 shrink-0">
        <div className="flex items-center gap-4"><Link href="/" className="bg-amber-800 hover:bg-amber-700 px-4 py-2 rounded-lg font-bold text-sm transition">🔙 ホーム</Link><h1 className="text-xl font-bold">{view === 'stock' ? '📂 ストック' : (filterMode === 'all' ? '📚 AIライブラリ' : '❤ お気に入り')}</h1></div>
        {view === 'read' ? <button onClick={() => { setView('shelf'); stopSpeaking(); }} className="text-sm bg-amber-800 px-3 py-1 rounded hover:bg-amber-700">本棚へ</button> : <button onClick={() => setShowMenu(true)} className="p-2 rounded hover:bg-amber-800 text-2xl">☰</button>}
      </header>
