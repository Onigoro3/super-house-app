...
export default function LibraryApp() {
  ...
  const [rawTexts, setRawTexts] = useState<RawText[]>([]); // ストック用データ
  ...
  // ファイルをアップロードしてストックに保存
  const uploadToStock = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      const { error } = await supabase.from('raw_texts').insert([{ title, content: text }]);
      if (!error) {
        alert(`「${title}」をストックに保存しました！`);
        fetchRawTexts();
        e.target.value = '';
      } else { alert('保存失敗'); }
    };
  ...
  const convertStockToBook = async (raw: RawText) => {
    if (!confirm(`「${raw.title}」を本棚に追加しますか？`)) return;
    
    // ページ分割ロジック
    const charsPerPage = 300; 
    const pages: BookPage[] = []; 
    let pageNum = 1;
    for (let i = 0; i < raw.content.length; i += charsPerPage) {
      const chunk = raw.content.substring(i, i + charsPerPage);
      const headline = pageNum === 1 ? raw.title : `ページ ${pageNum}`;
      pages.push({ page_number: pageNum, headline: headline, content: chunk.trim() }); 
      pageNum++;
    }

    const { error } = await supabase.from('books').insert([{ 
      title: raw.title, topic: 'インポート', pages: pages 
    }]);
  ...
  const deleteStock = async (id: number) => {
    if (!confirm("この原稿を削除しますか？")) return;
    await supabase.from('raw_texts').delete().eq('id', id);
    fetchRawTexts();
  };
      ...
      <header className="bg-amber-900 text-white p-4 shadow-md flex justify-between items-center z-10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold">
            {view === 'stock' ? '📂 テキストストック' : (filterMode === 'all' ? '📚 AIライブラリ' : '❤ お気に入り')}
          </h1>
        </div>
        {view === 'read' ? (
        ...
        <div className="max-w-5xl mx-auto w-full h-full flex flex-col">
          ...
          {view === 'create' && (
            <div className="bg-white p-6 rounded-lg shadow-sm border border-amber-200">
              <div className="flex justify-between items-center mb-4"><h2 className="font-bold text-lg text-amber-900">✨ 新しい本を執筆する</h2><button onClick={() => setView('shelf')} className="text-sm text-gray-400">キャンセル</button></div>
              <div className="flex flex-col gap-4">
                <input type="text" value={topic} onChange={e => setTopic(e.target.value)} placeholder="テーマ (例: 宇宙の歴史)" className="border p-3 rounded-lg w-full bg-amber-50 focus:bg-white transition" />
          ...
          {view === 'shelf' && (
            <div className="pb-20">
              ...
              {displayedBooks.length === 0 && <div className="text-center text-gray-400 py-20"><p className="text-4xl mb-2">📭</p><p>{filterMode === 'fav' ? 'お気に入りの本がありません' : '本がありません'}</p></div>}
            </div>
          )}
          ...
          {view === 'stock' && (
            <div className="space-y-6">
              {/* アップロードエリア */}
              <div className="bg-white p-6 rounded-lg shadow-sm border-2 border-dashed border-green-300 text-center hover:bg-green-50 transition cursor-pointer relative">
                 ...
                 <p className="text-xs text-gray-500">青空文庫や小説などの .txt ファイル</p>
              </div>

              {/* ストック一覧 */}
              <div className="grid gap-3">
                {rawTexts.length === 0 && <p className="text-center text-gray-400 py-10">保存された原稿はありません</p>}
                {rawTexts.map(raw => (
                  <div key={raw.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex justify-between items-center">
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-gray-800 truncate text-lg">📄 {raw.title}</h4>
                      <p className="text-xs text-gray-400">登録日: {new Date(raw.created_at).toLocaleDateString()}</p>
                    </div>
                    <div className="flex gap-2">
                       {/* 本にするボタン */}
                       <button 
                         onClick={() => convertStockToBook(raw)}
                         className="bg-amber-600 text-white px-4 py-2 rounded-lg font-bold text-sm shadow hover:bg-amber-700"
                       >
                         📖 本にする
                       </button>
