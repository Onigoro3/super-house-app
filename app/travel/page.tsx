    ...
    try {
      ...
      for (const day of plan.schedule) {
        ...
        for (const spot of day.spots) {
          ...
          y -= 15;
          const meta = `費用: ${spot.cost}  /  距離(堺から): ${spot.distance}`;
          page.drawText(meta, { x: 60, y, size: 10, font: customFont, color: rgb(0.5, 0.5, 0.5) });
          y -= 15;
          if (spot.url) {
  ...
  return (
    <div className="min-h-screen bg-teal-50 flex flex-col h-screen text-gray-800">
      ...
      <div className="flex bg-teal-700 p-1">
        ...
        <button onClick={() => setActiveTab('history')} className={`flex-1 py-2 text-sm font-bold ${activeTab === 'history' ? 'bg-white text-teal-700' : 'text-teal-100 hover:bg-teal-600'}`}>📜 保存したプラン ({historyList.length})</button>
      </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-teal-100">
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1"><label className="block text-xs font-bold text-gray-500 mb-1">期間</label><select value={duration} onChange={e => setDuration(e.target.value)} className="w-full border p-3 rounded-lg bg-white"><option value="日帰り">日帰り</option><option value="1泊2日">1泊2日</option><option value="2泊3日">2泊3日</option></select></div>
                    <div className="flex-1"><label className="block text-xs font-bold text-gray-500 mb-1">移動</label><select value={transport} onChange={e => setTransport(e.target.value)} className="w-full border p-3 rounded-lg bg-white"><option value="車">🚗 車</option><option value="電車">🚃 電車</option></select></div>
                  </div>
                  <div className="flex gap-2">
                     <div className="flex-1"><label className="block text-xs font-bold text-gray-500 mb-1">人数</label><select value={people} onChange={e => setPeople(e.target.value)} className="w-full border p-3 rounded-lg bg-white">{[1,2,3,4,5].map(p => <option key={p} value={p}>{p}人</option>)}</select></div>
                  ...
                  <div><label className="block text-xs font-bold text-gray-500 mb-1">テーマ</label><input type="text" value={theme} onChange={e => setTheme(e.target.value)} placeholder="例：歴史、海鮮" className="w-full border p-3 rounded-lg" /></div>
                </div>
                <button onClick={generatePlan} disabled={isGenerating} className={`w-full py-4 rounded-xl font-bold text-white text-lg shadow transition ${isGenerating ? 'bg-gray-400' : 'bg-teal-500 hover:bg-teal-600'}`}>{isGenerating ? 'AIがプランを作成中...' : '✨ 最高のプランを作成！'}</button>
              </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden animate-fadeIn">
                  <div className="bg-teal-600 text-white p-6 text-center relative">
                    ...
                    <div className="absolute top-4 right-4 flex gap-2">
                      <button onClick={savePlanToHistory} className="bg-white/20 text-white px-3 py-1.5 rounded-lg font-bold text-xs hover:bg-white/30 border border-white/50">💾 履歴に保存</button>
                      <button onClick={savePDF} disabled={isSaving} className="bg-white text-teal-700 px-3 py-1.5 rounded-lg font-bold text-xs shadow hover:bg-gray-100">{isSaving ? '...' : '📄 PDF'}</button>
                    </div>
                  </div>
                              ...
                              <div className="flex-1 bg-gray-50 p-4 rounded-xl border border-gray-100 group-hover:border-teal-200 transition">
                                <div className="flex justify-between items-start mb-1">
                                  <h4 className="font-bold text-teal-800 flex items-center gap-2">
                                    {spot.name}
                                    {spot.url && <a href={spot.url} target="_blank" rel="noreferrer" className="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded hover:bg-blue-200">🔗Link</a>}
                                  </h4>
                                  <div className="text-right flex flex-col items-end">
                                    <span className="text-xs bg-white border px-2 py-1 rounded text-gray-500 mb-1">{spot.cost}</span>
                                    {spot.distance && <span className="text-xs text-teal-600 font-bold">堺から {spot.distance}</span>}
                                  </div>
                                </div>
