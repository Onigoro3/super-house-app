...
export default function StockList({ view }: { view: ViewType }) {
  ...
  const saveAiRecipe = async (dish: Dish) => {
    const { error } = await supabase.from('recipes').insert([{
      ...
      calories: 0 // dishにカロリーがない場合もあるので0
    }]);
    ...
    return (
      <div className="p-4 space-y-8 pb-24 relative">
        ...
        <div className="bg-white p-5 rounded-xl border shadow-sm">
          ...
          <button onClick={generateMenu} disabled={selectedIds.length === 0 || loading} className={`w-full py-3 rounded-lg font-bold text-white shadow ${loading ? 'bg-indigo-400' : 'bg-indigo-600 hover:bg-indigo-700'}`}>{loading ? 'AIシェフが考案中...' : `✨ ${servings}人分の献立を5案考える！`}</button>
        </div>
  ...
  return <div className="p-4 space-y-6 pb-24"><h2 className="text-xl font-bold text-gray-800 border-l-4 border-blue-500 pl-3">{view === 'food' ? '🍎 食材リスト' : view === 'seasoning' ? '🧂 調味料リスト' : '🧻 日用品リスト'}</h2>{shoppingList.length > 0 && <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-200"><h3 className="font-bold text-yellow-800 mb-2">🛒 買うもの</h3><ul className="space-y-1">{shoppingList.map(i => <li key={i.id} className="flex justify-between text-sm bg-white px-2 py-1 rounded"><span>{i.name}</span><span className="text-gray-400">{i.quantity}</span></li>)}</ul></div>}<div className="bg-white p-4 rounded-xl border shadow-sm flex flex-col gap-3"><input value={newItemName} onChange={e => setNewItemName(e.target.value)} placeholder="品名を追加" className="w-full border p-3 rounded-lg text-black bg-gray-50 focus:bg-white" /><div className="flex gap-2 h-12"><input type="number" value={newItemCount} onChange={e => setNewItemCount(e.target.value)} placeholder="数" className="w-16 border p-2 rounded-lg text-black text-center bg-gray-50" /><select value={newItemUnit} onChange={e => setNewItemUnit(e.target.value)} className="w-20 border p-2 rounded-lg text-black bg-gray-50"><option value="個">個</option><option value="g">g</option><option value="ml">ml</option><option value="本">本</option><option value="束">束</option><option value="袋">袋</option><option value="パック">パック</option><option value="枚">枚</option><option value="玉">玉</option><option value="缶">缶</option><option value="箱">箱</option></select><button onClick={addItem} className="flex-1 bg-blue-600 text-white rounded-lg font-bold">＋</button><label className={`w-12 flex items-center justify-center bg-green-600 text-white rounded-lg font-bold cursor-pointer ${isAnalyzing?'opacity-50':''}`}><span>{isAnalyzing?'...':'📷'}</span><input type="file" accept="image/*" capture="environment" onChange={handleImageUpload} className="hidden" disabled={isAnalyzing} /></label></div></div><div className="space-y-2">{displayItems.map(item => <StockItem key={item.id} item={item} isEditing={editingId === item.id} editName={editName} editQuantity={editQuantity} setEditName={setEditName} setEditQuantity={setEditQuantity} onSave={saveEdit} onCancel={() => setEditingId(null)} onEditStart={() => startEditing(item)} onToggleStatus={toggleStatus} onDelete={deleteItem} />)}</div></div>;
}
