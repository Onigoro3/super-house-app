      ...
      for (let i = 0; i < currentBook.pages.length; i++) {
        ...
        if (imgBuffer) {
          try {
            ...
            page.drawImage(image, { x: (width - 500) / 2, y: height - 400, width: 500, height: 350 });
            ...
            <div className="flex flex-col h-full bg-white rounded-2xl shadow-2xl overflow-hidden border-4 border-pink-200">
              ...
              <div className="bg-white p-6 min-h-[150px] flex flex-col justify-center items-center text-center relative border-t-2 border-pink-100">
                ...
                <div className="absolute bottom-4 right-4 flex gap-2 items-center">
                  ...
                  {/* ★テキスト保存ボタン */}
                  <button onClick={downloadText} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded hover:bg-gray-200 border">📄 TXT</button>
                  
                  {/* ★PDF保存ボタン */}
                  <button onClick={savePDF} disabled={isSavingPDF} className="text-xs bg-pink-100 text-pink-600 px-2 py-1 rounded font-bold hover:bg-pink-200 border border-pink-200">
                    {isSavingPDF ? '...' : '📑 PDF'}
                  </button>
